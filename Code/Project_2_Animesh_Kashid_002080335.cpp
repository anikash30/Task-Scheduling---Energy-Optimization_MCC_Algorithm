...
struct task {
  ...
  double pri; // the priority level of the task
  ...
  int ready1; //the number of immediate predecessors of task which have not been scheduled
  int ready2; //the number of tasks in the same channel before the task which have not been scheduled
};
...
int main()
{
  ...
  int t1;
  int tmax1;
  std::cout<<"the number of tasks :: ";
  std::cin>>N1;
  std::cout<<"the maximum time to which the scheduling id to be restricted :: ";
  std::cin>>tmax1;
  std::cout<<"the power of core 1 :: ";
  std::cin>>p11;
  std::cout<<"the power of core 2 :: ";
  std::cin>>p12;
  std::cout<<"the power of core 3 :: ";
  std::cin>>p13;
        std::cout<<"the power of sending wireless :: ";
      std::cin>>ps1;
        double rt;
        clock_t start, end;
  t1 = ts1 + tc1 + tr1;
  Matrix<int, 2>G1(N1,N1);
  Matrix<int, 2>ta1(N1,3);
  vector<task>ini1(N1);
  vector<task>S1;
  int l;
  std::cout<<"the no. of graph connections ::";
  std::cin>>l;
  int h,k;
  for(int i=0;i<l;i++){
      std::cout<<"the starting node ";
      std::cin>>h;
      std::cout<<"the ending node ";
      std::cin>>k;
      G1(h,k) = 1;
  }
  ...
  for(int i=0;i<N1;i++){
      for(int j=0;j<3;j++){
      std::cout<<"the time for task "<<i+1<<" in core "<<j+1;
      std::cin>>ta1(i,j);
      }
  }
  ...
  cout << "The running time of initial schedule of Graph is "<<rt<<" ms"<< endl;
  start = clock();
